...
- 公開可讀；建立、編輯、刪除地區與事件都需輸入同一把管理員金鑰。建立時另保留 Turnstile 驗證。金鑰只留在當次頁面的記憶體，不寫入瀏覽器儲存。新增 API 也必須附上 `Authorization: Bearer <ADMIN_TOKEN>`。
...
- 匯出紀錄必須先由後端驗證管理員金鑰；錯誤金鑰不會觸發下載。公開閱讀 API 仍維持開放，因此此限制不代表公開資料無法被複製。
